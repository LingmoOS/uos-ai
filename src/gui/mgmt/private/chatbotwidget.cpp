...
DBackgroundGroup *ChatBotWidget::createPlatformGroup()
{
    ...
    const QString descText = tr("After enabling, UOS AI will receive messages from Lark, DingTalk, QQ, and WeChat through the configured bot. You can then directly interact with UOS AI in your IM client.");
...
void ChatBotWidget::onConfigureClicked(const QString &key)
{
    ...
    dlg.setConfig(existing);

    if (dlg.exec() != QDialog::Accepted)
        return;

    if (key == "wechat" && dlg.unbindRequested()) {
        QJsonObject cleared;
        cleared["enabled"] = false;
        platforms[key] = cleared;
        m_config["platforms"] = platforms;
        if (m_platformRows.contains(key))
            m_platformRows[key].row->setChecked(false);
        saveToService();

        qCInfo(logAIGUI) << "WeChat binding cleared";
        return;
    }
