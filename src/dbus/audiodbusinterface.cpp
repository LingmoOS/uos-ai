...
constexpr char kAudioService[] = "com.deepin.daemon.Audio";
constexpr char kAudioPath[] = "/com/deepin/daemon/Audio";
#else
constexpr char kAudioService[] = "org.deepin.dde.Audio1";
constexpr char kAudioPath[] = "/org/deepin/dde/Audio1";
#endif
}
...
AudioDbusInterface::AudioDbusInterface(QObject *parent)
    : QObject(parent)
{
    m_audioInter = new Audio(kAudioService, kAudioPath, QDBusConnection::sessionBus(), this);
...
bool AudioDbusInterface::isDefaultInputDeviceValid()
{
    ...
    if (device.isNull()) {
        qCWarning(logDBus) << "Default input device is null";
        return false;
    }

    if (!m_defaultSource) {
        qCWarning(logDBus) << "Default source proxy is not ready, fallback to Qt default input:" << device.description() << device.id();
        return true;
    }

    const QString qtDescription = device.description().trimmed();
    const QByteArray qtId = device.id();
    const QString sourceDescription = m_defaultSource->description().trimmed();
    const QString sourceName = m_defaultSource->name().trimmed();
    const bool matchesSource = (!qtId.isEmpty() && qtId == sourceName.toUtf8())
                               || (!qtDescription.isEmpty() && qtDescription == sourceDescription);
    if (!matchesSource) {
        qCDebug(logDBus) << "Default input device mismatch"
                         << "qtDescription:" << qtDescription
                         << "qtId:" << qtId
...
#else
    if (!m_defaultSource) {
        qCWarning(logDBus) << "Default source proxy is not ready";
        return false;
    }
    ...
    if (activePort.name.isEmpty()) {
        qCWarning(logDBus) << "Active input port is not ready yet, accept Qt default input temporarily";
        return true;
    }
...
{
    if (!m_defaultSink) {
        qCWarning(logDBus) << "Default sink proxy is not ready";
        return false;
    }
    ...
    if (device.isNull()) {
        qCWarning(logDBus) << "Default output device is null";
        return false;
    }

    const QString qtDescription = device.description().trimmed();
    const QByteArray qtId = device.id();
    const QString sinkDescription = m_defaultSink->description().trimmed();
    const QString sinkName = m_defaultSink->name().trimmed();
    if ((!qtId.isEmpty() && qtId != sinkName.toUtf8())
            && (!qtDescription.isEmpty() && qtDescription != sinkDescription)) {
...
{
#ifdef COMPILE_ON_V20
    m_defaultSource.reset(new Source(kAudioService, defaultSource.path(), QDBusConnection::sessionBus(), this));
#else
    m_defaultSource.reset(new Source(kAudioService, defaultSource.path(), QDBusConnection::sessionBus(), this));
#endif
...
{
#ifdef COMPILE_ON_V20
    m_defaultSink.reset(new Sink(kAudioService, defaultSink.path(), QDBusConnection::sessionBus(), this));
#else
    m_defaultSink.reset(new Sink(kAudioService, defaultSink.path(), QDBusConnection::sessionBus(), this));
#endif
